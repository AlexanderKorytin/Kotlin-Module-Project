...
class MenuNote(val nameNote: String,
    ...
    lateinit var context: String// переменная для вывода только сожержания конкретной заметки, сразу после заголовка меню заметки (MenuNote).
    ...
    override fun create() {
        ...
        run("Заметка ${nameNote}.", mapOfMenuItemNote, context)
    }
